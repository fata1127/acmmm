...
## Q4: Edge Case Resolution
The Fleiss' Kappa score primarily validated the high consistency among human experts in providing few-shot examples. For expanded malicious prompts, we utilized a fine-tuned XLM-RoBERTa model to assess alignment with expert judgments, as detailed in Section 3.2, "Attack Preparation." 
## Q5: Real-World Scalability
...
## Q8: Short Context Window
For models with strict token limits (smaller parameters), weaker safety alignment due to scaling-law means less malicious context is needed for jailbreaking. For instance, Mistral required fewer rounds than Qwen for successful breaking (Figure 6).
